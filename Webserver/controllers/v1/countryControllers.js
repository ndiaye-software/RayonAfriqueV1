...
const createCountryName = asyncHandler (async (req, res) => {
  try {
    const { countryName } = req.body;

    const newcountryName = new CountryProducts({
      countryName,
    });
...
const getCountryList = asyncHandler (async (req, res) => {
  try {
    const countries = await CountryProducts.find();
...
const updateCountry = asyncHandler(async (req, res) => {
  ...
  const country = await CountryProducts.findById(id).exec();
  ...
  // Check for duplicate
  const duplicate = await CountryProducts.findOne({ countryName })
...
const deleteCountry = asyncHandler(async (req, res) => {
  ...
  const countryName = await CountryProducts.findById(id).exec();
